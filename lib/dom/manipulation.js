import {toElementList, eachElement, setProperty} from './utils';
...
export function hasClass (elements, className) {
    return toElementList(elements).every(element => element.classList.contains(className));
};


export function append (parents, element) {
    toElementList(parents).forEach(parent => parent.appendChild(element));
};


export function prepend (parents, element) {
    toElementList(parents).forEach(parent => parent.insertBefore(element, parent.firstChild));
};
